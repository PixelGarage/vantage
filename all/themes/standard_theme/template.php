...
/**
 * Add image styles to node content.
 */
function standard_theme_preprocess_node(&$vars) {
  if ($vars['type'] == 'schools') {
    // create all needed image styles for the schools images
    foreach ($vars['field_picture'] as $i => $image) {
      ...
      if (!file_exists($dest)) {
        ...
        if (isset($style['effects'])) {
          image_style_create_derivative($style, $image['uri'], $dest);
        }
      }
    }
  }
